...
{
    internal class WorkerWithProtoSerialization
    {
        private readonly IConfiguration _configuration;
        private readonly KafkaProtoAutoProducer _kafkaProducer;
        private readonly KafkaProtoAutoConsumer _kafkaConsumer;

        public WorkerWithProtoSerialization(
            IConfiguration configuration,
            KafkaProtoAutoProducer kafkaProducer,
            KafkaProtoAutoConsumer kafkaConsumer,
            KafkaProtoSerializer protoSerializer)
        {
            _configuration = configuration;
            _kafkaProducer = kafkaProducer;
            _kafkaConsumer = kafkaConsumer;

            kafkaProducer.SetRecordSerializer(protoSerializer);
            kafkaConsumer.SetRecordSerializer(protoSerializer);
        }

        public void RunProduce()
        {
            var topicName = _configuration.GetValue<string>("KafkaAutoCommit:Topics:RequestTopicName") + "8";
            ...
            Task.Run(async () =>
            {
                try
                {
                    ...
                    {
                        var message = new KafkaProtoMessage()
                        {
        ...
        private static void PostProducingMessage(string? topicName, KafkaProtoMessage message)
        {
        ...
        private void PerformMessage(ConsumeResult<string, KafkaProtoMessage> consumeResult)
        {
